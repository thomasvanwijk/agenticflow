...
- Parses Obsidian-specific frontmatter while preserving unquoted `[[Wiki Links]]`.
- AI attribution (configurable injected callouts like `> [!ai] Gemini`).
- Injects `contributors` automatically into frontmatter.

To enable these Obsidian features, set `ENABLE_OBSIDIAN_FEATURES=true` in your `.env`. Otherwise, it operates as a standard, clean markdown file server.
...
- `ENABLE_OBSIDIAN_FEATURES`: `true` or `false`
